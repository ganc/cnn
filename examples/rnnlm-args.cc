...
int main(int argc, char** argv) {
  ...
  if (argc != 3 && argc != 4 && argc != 6) {
    cerr << "argc: " << argc;
    cerr << "Usage: " << argv[0] << " corpus.txt dev.txt [model.params] or [LAYERS INPUT_DIM HIDDEN_DIM]\n";
    return 1;
  }
  if (argc == 6) {
    LAYERS = stoi(argv[3]);
  ...
  //else
  sgd = new SimpleSGDTrainer(&model, 0.0);
  ...
  unsigned dev_every_i_reports = 250;
  ...
  while(1) {
    ...
    sgd->status();
    cerr << "epoch=" << (lines / (double)training.size()) << " E=" << (loss / chars) << " ppl=" << exp(loss / chars) << endl;
